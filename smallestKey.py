...
def return_smallest_key(inputDict, n):
    ...
    res = 0
    sortedDict = {val[0]: val[1] for val in sorted(inputDict.items(), key=lambda x: (x[1], x[0]))}
    sortedDictList = list(sortedDict)
    if n==0 or n > len(sortedDictList):
        res = None
    else:
        res = sortedDictList[n - 1]
    return res
